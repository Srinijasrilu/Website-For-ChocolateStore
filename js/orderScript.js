...
$(document).ready( function(){
  ...
  function getContent(){
    ...
    if(cart.size() == 0){
      items.html("<div class=\"item_order\"><h1 class=\"order_style\">Please select some products<br />and add them to your cart</h1></div>");
    }
    else{
      items.html(str);
    }
  ...
  function getTotals(){
    ...
    str += "<div class=\"clear\"></div>";
    if(strItemTotal != 0.00){
     str += "<img src=\"./images/checkout.jpg\" id=\"checkout_button\" class=\"pointer1 button2 checkout_button\"alt= \"checkout button\" name=\"checkout_button\" width=\"185\" />";
	 str += "<br /><br /><a href=\"products.html\" id=\"add_item_2\" >Continue shopping</a>";
    }
  ...
  function submitOrder(){
	...
	if(!isNumber(phone.val())){
      errorLine.html("Phone number must be a number");
      ccExp2.focus();
      return;
    }

	if(!isNumber(phoneBilling.val())){
      errorLine.html("Billing phone number must be a number");
      ccExp2.focus();
      return;
    }
    ...
    if(ccNumber.val().length < 16){
      errorLine.html("Credit card number must contain at least 16 digits");
      ccNumber.focus();
      return;
    }
