...
$(document).ready(function() {
	...
	var searchbutton = $("[id='searchicon']");
	...
	function openCartDialog(sku) { 
		...
		for(var i=0; i<proj4_data.length; i++){
			if(sku == proj4_data[i][0]){
				$("[id='add_to_cart_dialog']").html("Add "+proj4_data[i][2]+" to cart?");
			}
		}
...
function doSearch(token){
	...
	for(var i=0; i<proj4_data.length; i++){
		if(proj4_data[i][1] != null && proj4_data[i][1].toLowerCase().indexOf(token.toLowerCase()) > -1) {
		tmpString += "<div class=\"item\">";
